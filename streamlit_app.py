...
12) 목차는 모든 글에 포함하고, 서론 뒤에 '## 목차'를 둡니다. 목차는 모바일 가독성을 위해 4~5개 항목으로 간결하게 구성하세요. 각 목차 항목은 핵심 명사·짧은 구 형태로 작성하고, 원칙적으로 12~20자 이내를 목표로 하세요. '~하는 방법', '~알아보기', '~대해서'처럼 서술형으로 길게 쓰지 마세요. 예: '환급금 대상 확인', '환급액과 지급일', '신청 방법', '직접 신청해본 후기'. 주요 H2는 '## 1. ...', H3는 필요할 때 '### 1-1. ...' 형식으로 번호를 붙이세요.
...
H2/H3 제목은 모바일 목차에 그대로 보일 수 있으므로 짧고 검색자가 바로 이해할 수 있는 표현을 사용하세요.
body_markdown 시작은 2~4개의 서론 문단이어야 합니다.
서론 다음에는 반드시 "## 목차"를 넣고 1., 2., 3. 형식의 목차 항목을 작성하세요.
목차 항목은 실제 본문의 H2 소제목과 동일한 제목을 사용하세요. 목차는 4~5개만 만들고, H2 제목은 24자 이내를 원칙으로 하며 가능하면 12~20자로 압축하세요. 긴 문장형 제목, 콜론(:)으로 정보를 여러 개 나열하는 제목, 연도·수식어가 과도하게 붙은 제목은 피하세요. H2 제목 자체도 목차에 그대로 노출되므로 짧고 핵심적인 표현을 사용하세요. 한 H2에 여러 정보를 억지로 넣지 말고, 핵심 주제만 남기세요. 본문의 주요 H2는 반드시 "## 1. ...", "## 2. ..."처럼 번호를 붙이고, H3는 필요할 때만 "### 1-1. ..."처럼 번호를 붙이세요.
