...
def generate_one_article(client, topic: str, post_type: str, use_search: bool):
    """소재 하나를 받아 (선택)리서치 → 제목 → 본문까지 한 번에 생성."""
    research = ""
    if use_search:
        research_prompt = f"""다음 건강 관련 주제에 대해 웹 검색으로 신뢰할 수 있는 일반적인 배경지식을 찾아 정리해주세요.

주제: {topic}

- 효능을 과장하거나 특정 질병 치료를 단정하는 표현은 쓰지 마세요.
- 5~8줄 정도로 핵심만 정리해주세요."""
        research = ask_gemini(client, research_prompt, max_output_tokens=1200, use_search=True)

    type_guide_title = (
        "경험과 공감 위주로" if "홈판형" in post_type else "검색해서 들어온 사람이 필요한 답을 빠르게 찾도록"
    )
    title_prompt = f"""티스토리 건강 블로그에 올릴 글의 제목을 짓는 카피라이터입니다.
주제: {topic}
배경지식: {research or '없음'}
방향: {type_guide_title} 제목을 지어주세요.

가장 검색 노출에 유리하면서 클릭하고 싶은 제목 1개만 출력하세요. 다른 설명은 붙이지 마세요.
과장된 효능 단정 표현(완치, 100% 등)은 쓰지 마세요."""
    title = ask_gemini(client, title_prompt, max_output_tokens=200).strip().strip('"')
    ...
    body_prompt = f"""티스토리 건강 블로그 글을 쓰는 작가입니다.
제목: {title}
주제: {topic}
배경지식: {research or '없음'}

{type_guide_body} 900~1300자 분량의 본문을 작성하세요.
...
- 본문만 출력하세요."""
    body = ask_gemini(client, body_prompt, max_output_tokens=2000)

    return {"topic": topic, "research": research, "title": title, "body": body}
...
with st.sidebar:
    ...
    use_search_default = st.checkbox("웹 리서치 사용 (실제 검색 결과 기반으로 작성)", value=True)
...
st.caption("소재 하나씩 만들거나, 여러 주제를 한 번에 순차 생성할 수 있습니다. 발행은 완성된 원고를 복사해서 직접 붙여넣는 방식입니다.")

for key in ["research", "titles", "chosen_title", "body", "image_keywords", "batch_results"]:
...
with tab_single:
    ...
    clip_text = st.text_area(
        ...
        placeholder="예: [예고] 굽은 등 펴주는 5분 스트레칭, 오늘 밤 공개!",
        height=100,
    )
...
# =========================================================
# 탭 2: 배치 - 주제 목록을 한 번에 순차 생성
# =========================================================
with tab_batch:
    st.caption("왼쪽에서 선택한 글쓰기 유형·웹 리서치 설정을 그대로 사용해 순차 생성합니다.")
    ...
    if batch_input.strip():
        st.caption(f"{len(topics)}개 주제가 입력됐어요." + (f" (최대 {MAX_BATCH_TOPICS}개까지만 처리됩니다)" if len(batch_input.strip().split(chr(10))) > MAX_BATCH_TOPICS else ""))

    if st.button("🚀 전체 순차 생성", type="primary", disabled=not topics):
        st.session_state.batch_results = []
        progress = st.progress(0, text="시작합니다...")
        for i, topic in enumerate(topics):
            progress.progress(i / len(topics), text=f"({i+1}/{len(topics)}) '{topic}' 작성 중...")
            try:
                result = generate_one_article(client, topic, post_type, use_search_default)
                st.session_state.batch_results.append(result)
            except Exception as e:
                st.session_state.batch_results.append({"topic": topic, "error": str(e)})
    ...
    if st.session_state.batch_results:
        ...
        for i, result in enumerate(st.session_state.batch_results):
            ...
            with st.expander(f"✅ {result['title']}"):
                ...
                if result["research"]:
                    st.markdown("**배경지식**")
                    st.info(result["research"])
                st.markdown("**제목**")
                st.code(result["title"], language=None)
                st.markdown("**본문**")
                st.code(result["body"], language=None)

        st.caption("각 글의 제목/본문을 복사해서 티스토리 편집기에 붙여넣어 주세요. 이미지는 '하나씩 만들기' 탭에서 본문을 붙여넣어 따로 생성할 수 있어요.")
